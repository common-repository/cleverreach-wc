...
class CurlHttpClient extends HttpClient
{
    ...
    protected function executeAsynchronousRequest()
    {
        ...
        if ($statusCode === 0 && curl_errno($this->curlSession) === CURLE_OPERATION_TIMEOUTED) {
            $this->getConfigService()->setAsyncRequestTimeout(10000);
            $this->setCurlSessionOptionsForAsynchronousRequest();
            $this->setCurlOptions();
            list($result, $statusCode) = $this->executeCurlRequest();
            ...
            if (($statusCode === 0 && curl_errno($this->curlSession) === CURLE_OPERATION_TIMEOUTED) ||
                ...
                $this->getConfigService()->setAsyncRequestTimeout(self::DEFAULT_ASYNC_REQUEST_TIMEOUT);
    ...
    protected function logCurlError($statusCode, $result)
    {
        ...
        Logger::logInfo('Async process failed. ERROR: ' . $httpError);
    }
